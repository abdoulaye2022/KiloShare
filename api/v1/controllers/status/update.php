...
if(!$helper->isValidInteger($params['id'])) {
    $error = [
        "success" => false,
        "status" => 400,
        "message" => $errorHandler::getMessage('invalid_status_id')
    ];
    http_response_code(400);
    header('Content-Type: application/json');
    echo json_encode($error);
...
$updated = $statusModel->update($id, $name);
if($updated == false) {
	$error = [
        "success" => false,
        "status" => 400,
...
$statusFetch = $statusModel->getOne($id);
...
$status = $statusFetch->fetch(PDO::FETCH_ASSOC);
